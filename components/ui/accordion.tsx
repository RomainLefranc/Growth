...
const AccordionItem = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Item>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Item>>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item ref={ref} className={cn("border-b border-[#DFDEE2] transition-all data-[state=open]:border-black", className)} {...props} />
));
  ...
  ({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ref={ref}
        className={cn("flex flex-1 justify-between pb-6 text-left text-2xl font-normal leading-[1.5] no-underline transition-all [&[data-state=open]>svg]:rotate-[135deg]", className)}
        {...props}>
        {children}
...
const AccordionContent = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Content>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Content>>(
  ({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Content
      ref={ref}
      className={cn("overflow-hidden text-base font-normal leading-[1.6] transition-all data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down", className)}
      {...props}>
      <div className="mb-12 pt-0">{children}</div>
    </AccordionPrimitive.Content>
  )
);
